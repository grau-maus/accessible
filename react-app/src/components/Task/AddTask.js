...
const AddTask = () => {
  ...
  useEffect(() => {
    if (userList.length) setStaffId(Object.values(userList)[0].id);
    if (patientList.length) setPatientId(patientList[0].id);
  }, [userList.length, patientList.length]);
  ...
  return (
    ...
    >
      <div onClick={handleShow}>Add task</div>
      ...
      <Modal
        ...
        keyboard={false}
      >
        <Modal.Header closeButton className="modal-header-task">
